"""Candidate enumeration and three-objective Pareto ranking.
...
def sample_positive_partition(
    ...
    Not a uniform sampler over compositions. The k-1 cut points are drawn with
    replacement and then sorted, which favors interior partitions over extreme
    ones: for total = 6, k = 3 the most frequent partition appears about twice
    as often as the least. The behavior is kept because it is what enumerated
    the candidate pool the campaign ranked; it means the pool is a biased
    sample of the grid, not a census of it.
