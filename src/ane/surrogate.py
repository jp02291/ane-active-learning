...
The Optuna search picks the architecture and the weight given to
...
Ensemble construction reads the same parameters and builds the ensemble the
*selection* stage uses. The two are not the same model and are not used for the same
...
transformed with them, never fit on. That is the claim made in Methods (active
learning with ensemble neural networks) and it holds here.
...
def _fold_arrays(
    ...
    This is the step Methods describes. The scalers see the fold's
...
def cv_objective_loss(
    ...
    the search defines. Optuna then abandons that region.
    """
    import tensorflow as tf
    ...
    for train_idx, val_idx in kfold.split(X_comp):
        ...
        if len(losses):
            ...
            if not np.isfinite(best):
                ...
                # objective the search defines and would flatter a set that
