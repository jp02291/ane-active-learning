...
def evaluate_on_test(
    ...
    """Evaluate the pruned ensemble on the untouched test split.

    These are the numbers reported in the manuscript. The test split is not
    used for generator training, hyperparameter search, member pruning or any
    other decision -- it is read exactly here, once, at the end.
    """
    import joblib
    import tensorflow as tf
...
def train_ensemble(cfg: SurrogateConfig, verbose: bool = True) -> dict[str, Any]:
    ...
    if verbose:
        print("[5/5] evaluating on the untouched test split")
