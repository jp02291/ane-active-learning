...
used once, to complete the initial dataset. Those rows carry a model-derived
kappa wherever the partition puts them, training or held out.

The radii it uses differ from those in `elements.py` because they come from a
different source, and both sets are reported in the supplementary material
...
class CallawayParams:
    ...
    not the default. Nothing here is evaluated away from 300 K, so the
