...
with f_{i,s} normalised inside the site, and the two sites combine as

    Gamma = n_Fe Gamma_Fe-site + n_X Gamma_X-site.

Two conventions are load-bearing and differ from the Callaway-Klemens
reconstruction of Supplementary Note S1, which is a separate calculation.

There is no (M_i / M_s)^2 weight inside the site sum and no (M_s / M)^2 weight
on the site combination. Note S1 carries both; this analysis does not, and the
two are reported on their own scales rather than compared numerically.
...
def site_gamma(site: dict[str, float], prop: dict[str, float]) -> float:
    """Gamma_s(P) for one sublattice; the site composition is normalised here."""
