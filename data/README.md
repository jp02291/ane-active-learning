...
`reported_splits/` contains the recovered training and test membership of each
cycle: 36/9, 45/10 and 55/10 rows for cycles 1 to 3. `split/` is the output
directory written by `scripts/0_split_data.py`, which validates and copies
these memberships. The partition is not random; see `ane.data` for what it
does and why.
...
reconstructed thermal conductivity used as a training label, formed as described
in Supplementary Note S1; it is the value that appears in `data.csv` and in
