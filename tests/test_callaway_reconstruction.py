...
Reproduction. The deposited kappa_e and kappa_L must sum to the kappa that
appears in `data.csv` and in Supplementary Table S1, and kappa_L must be what
`kappa_lattice` returns for that composition.
"""
...
# ---------------------------------------------------------------------------
# The two parameterisations are one calculation
# ---------------------------------------------------------------------------

def test_si_equivalent_gamma_is_the_documented_ratio() -> None:
    """gamma_SI = gamma * sqrt(exp(2 theta_D / 3T)), because B is proportional to gamma^2."""
    ratio = np.exp(2 * CallawayParams.THETA_D / (3 * T))
    expected = CallawayParams.GAMMA * np.sqrt(ratio)
    assert CallawayParams.GAMMA_SI_EQUIVALENT == pytest.approx(expected, rel=1e-12)
    assert round(CallawayParams.GAMMA_SI_EQUIVALENT, 2) == 3.12, (
...
def test_both_parameterisations_give_the_same_lattice_conductivity(entries: pd.DataFrame) -> None:
    """The sign in Supplementary Note S1 and the sign in the code are not a disagreement."""
    for _, row in entries.iterrows():
        comp = _composition(row)
        as_implemented = kappa_lattice(comp, umklapp_sign=+1, gamma=CallawayParams.GAMMA)
        as_reported = kappa_lattice(
            comp, umklapp_sign=-1, gamma=CallawayParams.GAMMA_SI_EQUIVALENT
        )
        assert as_reported == pytest.approx(as_implemented, rel=1e-9), row["label"]
...
def test_the_signs_are_not_interchangeable_on_their_own() -> None:
    ...
    This is what makes the equivalence worth asserting rather than assuming.
    """
...
def test_electronic_term_follows_wiedemann_franz(entries: pd.DataFrame) -> None:
    for _, row in entries.iterrows():
        assert kappa_electronic(row["sigma_back_derived"], T, L0) == pytest.approx(
            row["kappa_e"], abs=1e-3
        ), row["label"]


def test_components_sum_to_the_reported_total(entries: pd.DataFrame) -> None:
    for _, row in entries.iterrows():
        assert row["kappa_e"] + row["kappa_L"] == pytest.approx(
            row["kappa_total"], abs=5e-3
        ), row["label"]
...
def test_back_derived_conductivities_are_metallic(entries: pd.DataFrame) -> None:
    """A weak but real check on the inversion: the values have to be plausible."""
    sigma = entries["sigma_back_derived"].to_numpy(float)
    assert sigma.min() > 5e5 and sigma.max() < 3e6
