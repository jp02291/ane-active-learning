"""The Fig. 4(d) projection must stay reproducible from the released data.
...
#: percentages reported for Fig. 4(d), to one decimal
PUBLISHED_VARIANCE = (31.0, 27.3)

#: the campaign enumerated candidates on a 0.01 grid (Methods: selection)
GRID_STEP = 0.01
...
def test_mean_centering_alone_would_change_the_figure(
    ...
    assert abs(centered[0] - PUBLISHED_VARIANCE[0]) > 5.0, (
        ...
        "Methods description of Fig. 4(d) would both need revisiting"
    )
